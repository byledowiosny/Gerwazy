!vectors.f90 - implementation of dynamically growing arrays.
!vectors_program.f90 - the way to use this.
!The module defines a suitable data structure. Data can be added to the vector and retrieve from it.
!Note: the function vector_at() needs a parameter that represents the "empty vector data" value.
type VECTOR
   private
   integer                                  :: no_used
...
contains
  ...
  !Create a new vector
  !Arguments:  vec           Variable that should hold the vector
  !            capacity      Initial capacity (optional)
  !Note: the fields of the vector data structure are set.
  ...
  !Destroy a vector
  !Arguments:  vec           Vector in question
  ...
  !Arguments:  vec           Variable that holds the vector
  ...
  !Arguments:  vec           Vector in question
  !            n             Index of the element whose value should be retrieved
  ...
  !Arguments:  vector        Vector in question
